...
export function GET({ props }: { props: { event: EventType } }) {
  const { event } = props;
  ...
  const countdownLabel = isPast ? "C'est arrivé !" : `${days} jour${days > 1 ? 's' : ''}`;
  const statusBadge = event.confirme ? 'Date confirmée' : 'Date estimée';
  ...
  // Build a clean 1200x630 OG image as SVG
  ...
  ${!isPast ? `<text x="600" y="475" text-anchor="middle" font-family="Inter, system-ui, sans-serif" font-size="22" fill="${c.texte_secondaire}">restants</text>` : ''}
